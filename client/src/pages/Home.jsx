...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const loader = async () => {
      let res = await axios.post("/api/user/getUsers", JSON.stringify({ filter: inputValue }), customConfig)
      ...
      <div className="flex  flex-col items-center w-[100%]">
        <div className="w-[50%] flex  flex-col items-center">
          <input
            ...
            type="text"
          />     <div className={`w-[60%] h-[200px] searchResults 
            ...
            <div className="overflow-y-scroll flex flex-col gap-2 items-center searchResultsDiv w-[100%]">
              {
                (users) ? (users.map((user) => {
                  return (
                    ...
                    <div key={user.username} onClick={()=>{send(user.username)}}  className="  px-2 w-[90%] bg-[#141619] text-white flex justify-between items-center rounded-lg">
                      {user.username}
                      {<img
