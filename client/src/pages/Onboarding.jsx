...
export default function Onboarding() {
  ...
  const techStackAdder = (e) => {
    if (!(formData.techStack).includes(e.target.value)) {
      ...
      e.target.className = "rounded-md border p-2 hover:scale-105 duration-200 hover:bg-green-400   border-[#818181]";
    }
  ...
  return (
    <div className='dashboardParent h-[93vh]
    w-[100%] p-5 flex flex-col gap-2 bg-black'>
      <div className={` addProjectCard dashboardChild  w-[90%] max-[767px]:w-[90%] rounded-[10px]
          border border-black
           border-dashed overflow-hidden`}>
        <form onSubmit={handleSubmit} className='text-white dashboardChildChild  flex flex-col items-center rounded-[10px] h-[80vh] p-4 gap-2 overflow-y-scroll'>
          ...
          <div className='max-[767px]:flex-col flex justify-between items-center w-[60%]'>
            ...
            <select onChange={handleChange} id="tags" ref={tagRef} className='w-[60%] bg-[#292929] border-2 border-[#3e3e3e] rounded-lg text-white px-6 py-3 text-base hover:border-[#fff] cursor-pointer transition'>
...
" onClick={() => { settag("AI Aficionado") }}>AI Aficionado</option>
          ...
          <div className='max-[767px]:flex-col flex justify-between items-center w-[60%]'>
            ...
            <textarea id="description" onChange={handleChange} ref={descRef} type='text' className='max-[767px]:w-[100%] w-[70%] resize-none aspect-video text-wrap bg-[#292929] border-2 border-[#3e3e3e] rounded-lg text-white px-6 py-3 text-base hover:border-[#fff] cursor-pointer transition' maxLength={100} placeholder='Describe yourself in 100 characters.' />
          </div>
          <div className='max-[767px]:w-[100%] max-[767px]:flex-col flex justify-between items-center w-[60%]'>
            ...
            {techs.map((tech) => {
                return (
                  <button key={tech} value={tech} onClick={(e) => { techStackAdder(e) }} type='button' className={`rounded-md border p-2 hover:scale-105 duration-200 hover:bg-green-400   border-[#818181]`}>
                    {tech}
                  </button>
                )
              })}
